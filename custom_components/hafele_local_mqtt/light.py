...
async def async_setup_entry(
    ...
    group_mode = entry.data.get("group_mode", "mqtt")  # Default to "mqtt" for backward compatibility
    enable_groups = entry.data.get("enable_groups", True)
    
    if enable_groups:
        if group_mode in ("mqtt", "both"):
            # Create MQTT-controlled group entities
        ...
        if group_mode in ("ha", "both"):
            # Create HA helper groups
